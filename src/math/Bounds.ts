...
const createDirectionalBounds = ({
  start,
  center,
  end,
  size,
}: DirectionalOptions): DirectionalBounds => {
  const blankFieldsCount = [
    start === undefined,
    center === undefined,
    end === undefined,
    size === undefined,
...
export const createBounds = (options: VerticalOptions & HorizontalOptions): Bounds => {
  const verticalValues = createDirectionalBounds({
    start: options.top,
    end: options.bottom,
    center: options.middle,
    size: options.height,
  })
  const horizontalValues = createDirectionalBounds({
    start: options.left,
    end: options.right,
